...
public class MyTranslations extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_my_translations);

        getSupportActionBar().hide();

        DatabaseManager helper = new DatabaseManager(this);
        SQLiteDatabase db = helper.getWritableDatabase();
        Cursor cursor_question = db.query("TRANSLATION", new String[]{"DATE","ORIGINAL_TEXT","TRANSLATED_TEXT","ID"},null,null,null,null,null);

        while(cursor_question.moveToNext()){
            String date = cursor_question.getString(0);
            String original_text = cursor_question.getString(1);
            String translated_text = cursor_question.getString(2);
            int id_translation = cursor_question.getInt(3);


            MyTranslation myTranslation = new MyTranslation(date,original_text,translated_text,id_translation);
